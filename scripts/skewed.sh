...
input_path=/remote_dataset/workloads/200M
		...
		for skew in $skew_factor; do 
			for idx in $index; do
				for t in 64 128; do
					...
					./bin/workload --input $input_path --workload $wk --threads $t --key_type rand --index $idx --hyper --skew $skew --earliest --num 200 >> ${output_path}/${wk}/${idx}_${skew_factor}
				done
			done
		done
	done
