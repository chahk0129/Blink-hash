...
for iter in $iterations; do
        for wk in $workloads; do
			...
			for t in $threads; do
				...
				./bin/adms --workload $wk --num 100000000 --threads $t --index $idx --type 1 >> ${output_art}/${idx}_${wk}
				rm zipfian_data_*.bin
                               	echo "---------------- running with threads $t ----------------" >> ${output_bwtree}/${idx}_${wk}
				./bin/adms --workload $wk --num 100000000 --threads $t --index $idx --type 2 >> ${output_bwtree}/${idx}_${wk}
				rm zipfian_data_*.bin
			done
		done
        done
