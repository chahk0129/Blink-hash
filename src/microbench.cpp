...
inline void exec(int wl, int index_type, int num_thread, kvpair_t<keytype>* init_kv, int init_num, kvpair_t<keytype>* run_kv, int run_num, int* ranges){
    ...
    auto func_upsert_earliest = [num_thread, idx, run_kv, run_num, &earliest_finished, &op_num, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
    ...
    auto func_upsert = [num_thread, idx, run_kv, run_num, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_op = run_num;
	size_t op_per_thread = total_num_op / num_thread;
	size_t start_index = op_per_thread * thread_id;
    ...
    auto func_read_earliest = [num_thread, idx, init_kv, run_num, &earliest_finished, &op_num, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_op = run_num;
	size_t op_per_thread = total_num_op / num_thread;
	size_t start_index = op_per_thread * thread_id;
    ...
    auto func_read = [num_thread, idx, init_kv, run_num, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_op = run_num;
	size_t op_per_thread = total_num_op / num_thread;
	size_t start_index = op_per_thread * thread_id;
    ...
    auto func_scan_earliest = [num_thread, idx, run_kv, run_num, ranges, &earliest_finished, &op_num, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_op = run_num;
	size_t op_per_thread = total_num_op / num_thread;
	size_t start_index = op_per_thread * thread_id;
    ...
    auto func_scan = [num_thread, idx, run_kv, run_num, ranges, &local_run_latency, &params, &perf_block, &breakdown](uint64_t thread_id, bool) {
	auto random_bool = std::bind(std::bernoulli_distribution(sampling_rate), std::knuth_b());
	size_t total_num_op = run_num;
	size_t op_per_thread = total_num_op / num_thread;
	size_t start_index = op_per_thread * thread_id;
