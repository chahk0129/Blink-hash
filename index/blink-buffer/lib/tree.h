#ifndef BLINK_BUFFER_TREE_H__
#define BLINK_BUFFER_TREE_H__
#include "node.h"
...
class btree_t{
	...
	restart:
	    ...
	    // debug
	    if(leaf->sibling_ptr)
		std::cout << "not rightmost node!" << std::endl;
	...
	void new_root_adjustment(Key_t* key, node_t** value, int num, node_t* prev){
	...
	restart:
	    auto cur = root;
	    if(cur == prev){ // prev is leaf and root
