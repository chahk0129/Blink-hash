...
public:
	...
	static inline HOTRowexSynchronizedIterator getBounded(HOTRowexChildPointer const * rootPointer, KeyType const & searchKey, bool isLowerBound, EpochBasedMemoryReclamationStrategy * const & memoryReclamationStrategy) {
		...
		if((currentRoot.isLeaf() && idx::contenthelpers::contentEquals(searchKey, extractKey(idx::contenthelpers::tidToValue<ValueType>(currentRoot.getTid())))) || currentRoot.isAValidNode()){
		    end = _rdtsc_();
		    if(!abort) time_traversal += (end - start);
		    else time_abort += (end - start);
		    return HOTRowexSynchronizedIterator(rootPointer, currentRoot, searchKey, isLowerBound, memoryReclamationStrategy, guard);
		}
		else{
		    end = _rdtsc_();
		    if(!abort) time_traversal += (end - start);
...
private:
	...
	inline HOTRowexSynchronizedIterator(HOTRowexChildPointer const * const & rootPointerLocation, HOTRowexChildPointer const & rootPointer, EpochBasedMemoryReclamationStrategy* memoryReclamationStrategy, MemoryGuard const & currentMemoryGuard) //DESCEND or STORE
		: mRootPointerLocation(rootPointerLocation),
	    	mMemoryReclamationStrategy(memoryReclamationStrategy), mCurrentBufferState(
			fillBuffer(
	...
	inline HOTRowexSynchronizedIterator(HOTRowexChildPointer const * rootPointerLocation, HOTRowexChildPointer const & rootPointer, KeyType const & searchKey, bool isLowerBound, EpochBasedMemoryReclamationStrategy* memoryReclamationStrategy, MemoryGuard const & currentMemoryGuard)
		: mRootPointerLocation(rootPointerLocation), mMemoryReclamationStrategy(memoryReclamationStrategy),
	    	mCurrentBufferState(fillBufferForBound(getBufferRoot(), getStackRoot(), rootPointerLocation, rootPointer, searchKey, isLowerBound, currentMemoryGuard)) {
	}
