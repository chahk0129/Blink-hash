...
class lnode_t : public node_t{
    public: 
	//static constexpr size_t leaf_size = 1024 * 8;
	...
	int insert(Key_t key, uint64_t value, uint64_t version){
		...
		for(int j=0; j<num_slot; j++){
			...
			for(int i=0; i<16; i++){
				...
				if(need_restart || (version != _version) || (sibling_ptr && (high_key < key))){
	...
	#else
	lnode_t<Key_t>* split(Key_t& split_key, Key_t key, uint64_t value, uint64_t version, uint64_t& lnode_alloc, uint64_t& lnode_sync, uint64_t& lnode_split, uint64_t& lnode_key_copy, uint64_t& lnode_find_median, uint64_t& lnode_copy, uint64_t& lnode_update){
...
PROCEED:
		bucket[j].mutex.unlock_non_atomic();
