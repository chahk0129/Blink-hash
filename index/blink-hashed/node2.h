...
class lnode_t : public node_t{
    public: 
	//static constexpr size_t leaf_size = 1024 * 16;
...
//#define DEBUG
		...
		for(int k=0; k<2; k++){
		    __m128i fingerprints_ = _mm_loadu_si128(reinterpret_cast<__m128i*>(bucket[j].fingerprints + k*16));
		    __m128i new_fingerprints = _mm_loadu_si128(reinterpret_cast<__m128i*>(new_right->bucket[j].fingerprints + k*16));
		    __m128i cmp = _mm_cmpeq_epi8(empty, fingerprints_);
		    __m128i finger_cmp = _mm_cmpeq_epi8(fingerprints_, new_fingerprints);
		    uint16_t bitfield = _mm_movemask_epi8(cmp);
		    uint16_t finger_bitfield = _mm_movemask_epi8(finger_cmp);
		    for(int i=0; i<16; i++){
			auto bit = (bitfield >> i);
			if((bit & 0x1) == 0){
			    auto idx = k*16 + i;
			    if(split_key < bucket[j].entry[idx].key){
				auto finger_bit = (finger_bitfield >> i);
				if((finger_bit & 0x1) == 0){
				    #ifdef BREAKDOWN
				    end = _rdtsc();
		...
		for(int i=0; i<entry_num; i++){
			...
			if(split_key < bucket[j].entry[i].key){
			    if(new_right->bucket[j].fingerprints[i] != bucket[j].fingerprints[i]){
