...
restart:
		...
		// collected all the values within a single leaf
		if(count == range){
		    node->readUnlockOrRestart(versionNode, needRestart);
		    if(needRestart) goto restart;
		    return count;
		...
		// leaf traversal
		while(count < range && leaf->sibling_ptr){ 
		    BTreeLeaf<Key,Value>* sibling = leaf->sibling_ptr;
		    uint64_t versionSibling = sibling->readLockOrRestart(needRestart);
		    if(needRestart) goto restart;

		    leaf->readUnlockOrRestart(versionNode, needRestart);
		    if(needRestart) goto restart;

		    for(unsigned i=0; i<sibling->count; i++){
			if(count == range) break;
			output[count++] = sibling->data[i].second;
		    }

		    sibling->readUnlockOrRestart(versionSibling, needRestart);
		    if(needRestart) goto restart;
