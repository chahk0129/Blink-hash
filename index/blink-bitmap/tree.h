#ifndef TREE_BITMAP_H__
#define TREE_BITMAP_H__
#include "node.h"
#include <stack>
...
class btree_t{
...
#ifdef TIME
	    uint64_t _time_internal_traverse, _time_internal_sync, _time_internal_write, _time_leaf_traverse, _time_leaf_sync, _time_leaf_write;
	    _time_internal_traverse = _time_internal_sync = _time_internal_write = _time_leaf_traverse = _time_leaf_sync = _time_leaf_write = 0;
	    uint64_t start, end;
#endif
	    auto cur = root;
	...
	restart:
	    ...
	    auto cur_vstart = cur->try_readlock_internal(need_restart);
	    if(need_restart){
#ifdef TIME
		end = _rdtsc();
		...
		start = _rdtsc();
#endif
		auto child_vstart = child->try_readlock_internal(need_restart);
		if(need_restart){
#ifdef TIME
		    end = _rdtsc();
		    _time_internal_sync += (end - start);
		...
		auto cur_vend = cur->get_version_internal(need_restart);
	...
	leaf_restart:
...
#endif
	    need_restart = false;
	    auto leaf_vstart = leaf->try_readlock_leaf_for_write(need_restart);
	    if(need_restart){
#ifdef TIME
		end = _rdtsc();
		...
		start = _rdtsc();
#endif
		auto sibling_v = sibling->try_readlock_leaf_for_write(need_restart);
		if(need_restart){
#ifdef TIME
		    end = _rdtsc();
		    _time_leaf_sync += (end - start);
		...
		if(!leaf->compare_version_for_write(leaf_vstart)){
		    
//		auto leaf_vend = leaf->get_version_leaf(need_restart);
//		if(need_restart || (leaf_vstart != leaf_vend)){
	    ...
	    start = _rdtsc();
#endif
	    leaf->try_upgrade_writelock_leaf(leaf_vstart, need_restart);
	    if(need_restart){
#ifdef TIME
		end = _rdtsc();
		_time_leaf_sync += (end - start);
	    ...
	    if(leaf->insert(key, value)){ // normal insert
#ifdef TIME
		end = _rdtsc();
		_time_leaf_write += (end - start);
		start = _rdtsc();
#endif
		leaf->write_unlock_leaf();
#ifdef TIME
		end = _rdtsc();
		_time_leaf_sync += (end - start);
		...
		if(stack_cnt){
...
#endif
			need_restart = false;
			auto parent_vstart = old_parent->try_readlock_internal(need_restart);
			if(need_restart){
#ifdef TIME
			    end = _rdtsc();
			    _time_internal_sync += (end - start);
			...
			while(old_parent->sibling_ptr && (old_parent->high_key < split_key)){
			    ...
			    start = _rdtsc();
#endif
			    auto p_sibling_v = p_sibling->try_readlock_internal(need_restart);
			    if(need_restart){
#ifdef TIME
				end = _rdtsc();
				_time_internal_sync += (end - start);
			    ...
			    auto parent_vend = old_parent->get_version_internal(need_restart);
			...
			old_parent->try_upgrade_writelock_internal(parent_vstart, need_restart);
			if(need_restart){
#ifdef TIME
			    end = _rdtsc();
			    _time_internal_sync += (end - start);
			...
			if(original_node->level != 0)
			    original_node->write_unlock_internal();
			else
			    original_node->split_unlock_leaf();
#ifdef TIME
			end = _rdtsc();
			if(original_node->level == 0)
			    _time_leaf_sync += (end - start);
			...
			if(!old_parent->is_full()){ // normal insert
			    ...
			    start = _rdtsc();
#endif
			    old_parent->write_unlock_internal();
#ifdef TIME
			    end = _rdtsc();
			    _time_internal_sync += (end - start);
			    update_time(_time_internal_traverse, _time_internal_sync, _time_internal_write, _time_leaf_traverse, _time_leaf_sync, _time_leaf_write);
	...
	restart:
		...
		start = _rdtsc();
#endif
		auto sibling_vstart = sibling->try_readlock_internal(need_restart);
		if(need_restart){
#ifdef TIME
		end = _rdtsc();
		_time_internal_sync += (end - start);
	    ...
	    start = _rdtsc();
#endif
	    cur->try_upgrade_writelock_internal(cur_vstart, need_restart);
	    if(need_restart){
#ifdef TIME
		end = _rdtsc();
		_time_internal_sync += (end - start);
...
#endif
	    if(prev->level != 0)
		prev->write_unlock_internal();
	    else
		prev->split_unlock_leaf();
#ifdef TIME
	    end = _rdtsc();
	    if(prev->level == 0)
		_time_leaf_sync += (end - start);
...
#ifdef ATOMIC_UPDATE
	    leaf->try_upgrade_updatelock_leaf(leaf_vstart, need_restart);
#else
	    leaf->try_upgrade_writelock_leaf(leaf_vstart, need_restart);
#endif
	    if(need_restart) goto leaf_restart;
	...
	leaf_restart:
		...
		*/
	    ...
	    if(!leaf->compare_version_for_read(leaf_vstart))
		goto leaf_restart;
	...
	restart:
	    ...
	    auto leaf_vstart = leaf->try_readlock_leaf(need_restart);
	    if(need_restart) goto restart;
		...
		while(leaf->sibling_ptr && (leaf->high_key < min_key)){
		    ...
		    auto sibling_v = sibling->try_readlock_leaf(need_restart);
		    if(need_restart) goto restart;
		...
		*/
		...
		auto sibling_vstart = sibling->try_readlock_leaf(need_restart);
		if(need_restart) goto restart;
