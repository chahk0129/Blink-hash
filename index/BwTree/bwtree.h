...
static thread_local uint64_t time_consolidation;
...
class BwTree : public BwTreeBase {
 ...
 public:
  ...
  #ifdef BREAKDOWN
  void get_breakdown(uint64_t& _time_traversal, uint64_t& _time_abort, uint64_t& _time_latch, uint64_t& _time_node, uint64_t& _time_split, uint64_t& _time_consolidation){
      _time_traversal = time_traversal;
      _time_abort = time_abort;
      _time_latch = time_latch;
      _time_node = time_node;
      _time_split = time_split;
      _time_consolidation = time_consolidation;
  }
  #endif
 ...
 public:
  ...
  void DebugDuplicateKeyNavigateLeafNode(Context* context_p, std::vector<ValueType>& key_addr){
      NavigateSiblingChain(context_p);
      if(context_p->abort_flag == true){
	  bwt_printf("NavigateSiblingChain in LeafNode aborted\n");
	  return;
      }
      ...
      const KeyType& key = context_p->search_key;
	  ...
	  switch(type){
		  ...
		  for(auto it=start_it; it!=end_it; it++){
		      if(KeyCmpEqual(key, it->first)){
			  key_addr.push_back(it->second);
		      }
		  }
		  return;
	      }
		  ...
		  if(KeyCmpEqual(key, insert_node_p->item.first)){
		      key_addr.push_back(insert_node_p->item.second);
		  }
	      ...
	      default:
		  printf("other leaf delta node types other than LeafType, LeafInsertType or LeafSplitType\n");
	  }
      }
  }
  ...
  bool DebugDuplicateKey(const KeyType& key){

      uint64_t abort_cnt = 0;
retry_traverse:
      Context context_p{key};
	  ...
	  if(context_p.abort_flag == true){
	      abort_cnt++;
	      printf("Navigate Inner Node aborted %llu\n", abort_cnt);
      ...
      std::vector<ValueType> key_addr;
      DebugDuplicateKeyNavigateLeafNode(&context_p, key_addr);
      if(key_addr.size() == 0){
	  printf("key not found!!\n");
	  return false;
      }
      else if(key_addr.size() != 1){
	  printf("duplicate keys!!\n");
	  
	  for(auto it=key_addr.begin(); it!=key_addr.end(); it++){
  ...
  void getMemory(const NodeID node_id, uint64_t& meta, uint64_t& structural_data_occupied, uint64_t& structural_data_unoccupied, uint64_t& key_data_occupied, uint64_t& key_data_unoccupied){
	  ...
	  structural_data_occupied -= sizeof(NodeID);
	  structural_data_unoccupied += sizeof(NodeID);
	  return;
      }
	  ...
	  switch(type){
	      case NodeType::LeafType:{
		  const LeafNode* leaf = static_cast<const LeafNode*>(node_p);
		  meta += sizeof(LeafNode);
		  for(auto it=leaf->Begin(); it!=leaf->End(); it++){
		      key_data_occupied += sizeof(KeyValuePair);
		  }
		  return;
	      }
	      case NodeType::LeafInsertType:{
		  const LeafInsertNode* leaf = static_cast<const LeafInsertNode*>(node_p);
		  meta += sizeof(BaseNode) + sizeof(std::pair<int, bool>);
		  key_data_occupied += sizeof(KeyValuePair);
		  structural_data_occupied += sizeof(const BaseNode*);
		  node_p = leaf->child_node_p;
		  goto leaf_delta;
	      }
	      case NodeType::LeafSplitType:{
		  const LeafSplitNode* leaf = static_cast<const LeafSplitNode*>(node_p);
		  assert(leaf->GetSize() == leaf->GetItemCount());
		  meta += sizeof(BaseNode);
		  structural_data_occupied += sizeof(KeyNodeIDPair);
		  //getMemory(leaf->insert_item.second, meta, structural_data_occupied, structural_data_unoccupied, key_data_occupied, key_data_unoccupied);
		  node_p = leaf->child_node_p;
		  goto leaf_delta;
	      }
	  ...
	  switch(type){
		  ...
		  meta += sizeof(InnerNode);
		  auto item_num = inner->GetItemCount();
		  structural_data_occupied += sizeof(KeyType)*item_num;
		  for(const NodeID* p = inner->NodeIDBegin(); p!=inner->NodeIDEnd(); p++){
		      structural_data_occupied += sizeof(NodeID);
		      getMemory(*p, meta, structural_data_occupied, structural_data_unoccupied, key_data_occupied, key_data_unoccupied);
		  }
		  return;
	      }
	      case NodeType::InnerInsertType:{
		  const InnerInsertNode* inner = static_cast<const InnerInsertNode*>(node_p);
		  meta += sizeof(BaseNode) + sizeof(int);
		  structural_data_occupied += sizeof(const BaseNode*) + sizeof(KeyNodeIDPair)*2;
		  getMemory(inner->item.second, meta, structural_data_occupied, structural_data_unoccupied, key_data_occupied, key_data_unoccupied);
		  //getMemory(inner->next_item.second, meta, structural_data_occupied, structural_data_unoccupied, key_data_occupied, key_data_unoccupied);
		  ...
		  meta += sizeof(BaseNode);
		  structural_data_occupied += sizeof(KeyNodeIDPair) + sizeof(const BaseNode*);
		  //getMemory(inner->insert_item.second, meta, structural_data_occupied, structural_data_unoccupied, key_data_occupied, key_data_unoccupied);
		  node_p = inner->child_node_p;
		  goto inner_delta;
	      }
