...
class betree_t{
	...
	void put(entry_t<Key_t, Value_t>* buf, int cnt){
	    int restart_cnt = 0;
	restart:
		...
		if(!inode->is_msg_full(cnt)){
		    inode->put_msg(buf, cnt);
		    inode->write_unlock();
		    return;
		}
	...
	restart:
		...
		//auto ret = flush(nullptr, dummy, inode, cur_vstart);
		auto ret = flush(nullptr, inode);
		goto restart;
	    }
	    else{ // leaf
