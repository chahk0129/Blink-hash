...
class betree_t{
	...
	int calculate_potential_split(child_info_t* child_info, int child_info_size){
	    ...
	    return potential_child_split;
	}


	bool flush(inode_t<Key_t, Value_t>* parent, uint64_t parent_version, bool& parent_lock, inode_t<Key_t, Value_t>* cur, uint64_t cur_version, bool& cur_lock){
	    bool need_restart = false;
	    ...
	    if(pivot_cnt > 3){
		if(parent && !parent_lock){
		    parent->try_upgrade_writelock(parent_version, need_restart);
		    if(need_restart) return false;
		    parent_lock = true;
		}

		if(!cur_lock){
		    cur->try_upgrade_writelock(cur_version, need_restart);
		    if(need_restart){
			if(parent){
			    parent->write_unlock();
			    parent_lock = false;
		...
		auto potential_child_split = calculate_potential_split(child_info, child_info_size);
		...
		auto child_version = child->get_version(need_restart);
		if(need_restart){// need to fix
		    cleanup_idx = from;
		    goto cleanup;
		}
		...
		if(child->level == 0){ // lnode
		    ...
		    _child->try_upgrade_writelock(child_version, need_restart);
		    if(need_restart){
			cleanup_idx = from;
			goto cleanup;
		    }
			...
			if(from > 0)
			    cur->move_msg(from);
			auto ret = flush(cur, cur_version, cur_lock, _child, child_version, child_lock);
			return false;
		    }
		}
	    }
	...
	restart:
		...
		bool cur_lock = false;
		auto ret = flush(nullptr, dummy_version, dummy_lock, inode, cur_vstart, cur_lock);
		goto restart;
	    }
	    else{ // leaf
