...
namespace ART_OLC {
    ...
    uint64_t N::readLockOrRestart(bool& needRestart, bool& isObsolete_) const{
	uint64_t version;
	version = typeVersionLockObsolete.load();
	needRestart = isLocked(version);
	isObsolete_ = isObsolete(version);
	return version;
    }
...
/*        do {
    ...
    uint64_t N::getChildren(const N* node, uint8_t start, uint8_t end, std::tuple<uint8_t, N*> children[], uint32_t& childrenCount, bool& isObsolete){
	switch(node->getType()){
	    case NTypes::N4:{
		auto n = static_cast<const N4*>(node);
		return n->getChildren(start, end, children, childrenCount, isObsolete);
	    }
	    case NTypes::N16:{
		auto n = static_cast<const N16*>(node);
		return n->getChildren(start, end, children, childrenCount, isObsolete);
