...
class lnode_t : public node_t{
	...
	static constexpr size_t cardinality = (1024*1024- sizeof(node_t) - sizeof(Key_t))/ sizeof(bucket_t<Key_t, uint64_t>);
	...
	bool _is_locked(uint64_t version){
	    return (version & ((uint64_t)1 << entry_num)) != 0;
	}

	uint64_t get_version(bool& need_restart, int loc){
	    uint64_t version = bucket[loc].token.load();
	    if(_is_locked(version)){ 
		_mm_pause();
		need_restart = true;
	    }
	    return version;
	...
	uint64_t _try_readlock(bool& need_restart, int loc){
	    uint64_t version = bucket[loc].token.load();
	    if(_is_locked(version)){
		_mm_pause();
		need_restart = true;
	...
	bool _compare_version(uint64_t version, int loc){
	    uint64_t _version = bucket[loc].token.load();
	    if(_is_locked(_version))
		return false;
	    if(version != _version)
		return false;
	    return true;
	...
	bool _try_writelock(int loc){
	    ...
	    if(!bucket[loc].token.compare_exchange_strong(version, version+((uint64_t)1 << entry_num))){
		_mm_pause();
		return false;
	    }
	    return true;
	...
	void _split_unlock(){
	    ...
	    for(int i=0; i<cardinality; i++){
		bucket[i].token.fetch_add((uint64_t)1 << entry_num);
	    }
	}

	void _write_unlock(int loc){
	    bucket[loc].token.fetch_add((uint64_t)1 << entry_num);
	}
	...
	void insert_after_split(Key_t key, uint64_t value){
		...
		*/
		...
		auto token = bucket[loc].token.load();
		for(int i=0; i<entry_num; i++){
		    int idx = (uint32_t)1 << i;
		    if((token & idx) == 0){
			bucket[loc].entry[i].key = key;
			bucket[loc].entry[i].value = value;
			bucket[loc].token.store(token | idx, std::memory_order_relaxed);
			return;
		    }
		}
	    }
	...
	lnode_t<Key_t>* split(Key_t& split_key){ 
		...
		uint64_t bitmap = (uint64_t)~0ul >> entry_num << entry_num;
		bucket[i].token.store(token & bitmap, std::memory_order_release);
	    }
	...
	restart:
		...
		loc = (loc + j) % cardinality;
		auto token = bucket[loc].token.load();
		if(_is_locked(token)) goto restart;
		...
		for(int i=0; i<entry_num; i++){
			...
			if(bucket[loc].entry[i].key == key){
			    ...
			    auto _token = bucket[loc].token.load();
			    if(_is_locked(_token) || token != _token)
				goto restart;
			    return ret;
			}
		    }
