...
int main(int argc, char* argv[]){
    ...
    int mode = atoi(argv[3]); // 0: try cas       1: cas till success
    ...
    data.store(0);
    auto cas_till_success = [&data](int num){
	for(int i=0; i<num; i++){
	    uint64_t _data = data.load();
    ...
    auto cas = [&data](int num){
	for(int i=0; i<num; i++){
	    uint64_t _data = data.load();
	    data.compare_exchange_strong(_data, _data+1);
	}
    };
	...
	start_threads(data, num_threads, cas, chunk);
	clock_gettime(CLOCK_MONOTONIC, &end);
    }
    else{
	std::cout << "doing cas until success... " << std::endl;
	clock_gettime(CLOCK_MONOTONIC, &start);
	start_threads(data, num_threads, cas_till_success, chunk);
	clock_gettime(CLOCK_MONOTONIC, &end);
    }
